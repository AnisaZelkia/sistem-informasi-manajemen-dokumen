...
class Home extends CI_Controller {
  ...
  protected function src($srcdata=false)
  {
	...
	public function index()
	{
		...
		if ($this->session->tipe == "admin" || $this->session->tipe == "operator") {
			$query_chart1 =  $this->db->query("SELECT COUNT(id) as count,MONTHNAME(tanggal) as month_name FROM data_arsip WHERE YEAR(tanggal) = '" . date('Y') . "' GROUP BY YEAR(tanggal),MONTH(tanggal)"); 
		} else {
			$query_chart1 =  $this->db->query("SELECT COUNT(id) as count,MONTHNAME(tanggal) as month_name FROM data_arsip WHERE YEAR(tanggal) = '" . date('Y') . "' and username='".$_SESSION['username']."' GROUP BY YEAR(tanggal),MONTH(tanggal)"); 
		}
